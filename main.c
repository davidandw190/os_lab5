...
void createChildProcesses(int num_processes, int parent_fd) {
    for (int i = 1; i <= num_processes; i++) {
        ...
        if (child_pid == 0) {
            ...
            printf(" ---> CHILD #%d, PID = %d, PARENT PID = %d, USER ID = %d, GROUP ID = %d\n", i, getpid(), getppid(), (int) getuid(), (int) getgid());
            exit(i);
            ...
            if (WIFEXITED(status)) {
                printf(" #### The parent received value %d from Child #%d and printed it to the file with FD %d. \n", WEXITSTATUS(status), i, parent_fd);
            } else {
